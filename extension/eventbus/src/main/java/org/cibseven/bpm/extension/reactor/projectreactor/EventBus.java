...
package org.cibseven.bpm.extension.reactor.projectreactor;
...
import org.cibseven.bpm.extension.reactor.projectreactor.dispatch.SynchronousDispatcher;
import org.cibseven.bpm.extension.reactor.projectreactor.filter.PassThroughFilter;
import org.cibseven.bpm.extension.reactor.projectreactor.publisher.BusPublisher;
import org.cibseven.bpm.extension.reactor.projectreactor.registry.CachingRegistry;
import org.cibseven.bpm.extension.reactor.projectreactor.registry.Registration;
import org.cibseven.bpm.extension.reactor.projectreactor.registry.Registries;
import org.cibseven.bpm.extension.reactor.projectreactor.registry.Registry;
import org.cibseven.bpm.extension.reactor.projectreactor.routing.ConsumerFilteringRouter;
import org.cibseven.bpm.extension.reactor.projectreactor.routing.Router;
import org.cibseven.bpm.extension.reactor.projectreactor.selector.ClassSelector;
import org.cibseven.bpm.extension.reactor.projectreactor.selector.Selector;
import org.cibseven.bpm.extension.reactor.projectreactor.selector.Selectors;
import org.cibseven.bpm.extension.reactor.projectreactor.spec.EventBusSpec;
import org.cibseven.bpm.extension.reactor.projectreactor.support.UUIDUtils;
...
public class EventBus implements Bus<Event<?>>, Consumer<Event<?>> {
  ...
  /**
   * Create a new {@link org.cibseven.bpm.extension.reactor.projectreactor.spec.EventBusSpec} to configure a Reactor.
   *
  ...
  /**
   * Create a new {@link EventBus} using the given {@link Dispatcher}.
   *
   * @param dispatcher The name of the {@link Dispatcher} to use.
   * @return A new {@link EventBus}
   */
  ...
  /**
   * Create a new {@link EventBus} using the given {@link Environment} and dispatcher name.
   *
   * @param env        The {@link Environment} to use.
   * @param dispatcher The name of the {@link Dispatcher} to use.
   * @return A new {@link EventBus}
  ...
  /**
   * Create a new {@link EventBus} using the given {@link Environment} and {@link
   * Dispatcher}.
   *
   * @param env        The {@link Environment} to use.
   ...
   * org.cibseven.bpm.extension.reactor.projectreactor.routing.Router} that broadcast events to all of the registered consumers that {@link
  ...
  public EventBus(@Nonnull Registry<Object, Consumer<? extends Event<?>>> consumerRegistry,
                  ...
                  @Nullable Consumer<Throwable> dispatchErrorHandler,
                  @Nullable final Consumer<Throwable> uncaughtErrorHandler) {
    requireNonNull(consumerRegistry, "Consumer Registry cannot be null.");
  ...
  /**
   * Get the {@link org.cibseven.bpm.extension.reactor.projectreactor.routing.Router} used to route events to {@link Consumer Consumers}.
   *
   * @return The {@link org.cibseven.bpm.extension.reactor.projectreactor.routing.Router}.
   */
  ...
  public EventBus notify(Object key, Event<?> ev) {
    ...
    requireNonNull(ev, "Event cannot be null.");
    ev.setKey(key);
  ...
  /**
   * Register the given {@link Consumer} on an anonymous {@link org.cibseven.bpm.extension.reactor.projectreactor.selector.Selector} and
   * set the given event's {@code replyTo} property to the corresponding anonymous key, then register the consumer to
   * receive replies from the {@link Function} assigned to handle the given key.
   *
   * @param key   The key to be matched by {@link Selector Selectors}
   ...
   * replies from the {@link Function} assigned to handle the given key.
   *
   * @param key      The key to be matched by {@link Selector Selectors}
  ...
  /**
   * Schedule an arbitrary {@link Consumer} to be executed on the current Reactor  {@link
   * Dispatcher}, passing the given {@param data}.
   *
